...
This map explores the variation in 2021 home prices across Philadelphia. We noticed the most expensive areas are located near Center City or far outskirts such as Chestnut Hill.   
...
reg.nhood <- lm(sale_price ~ ., data = as.data.frame(Philly.training) %>% 
                                 dplyr::select(sale_price, shooting_halfmile, interior_condition, total_livable_area, 
                                           ...
                                           DevelopmentArea, tree_count, medHHInc, 
                                           exterior_condition, fireplaces, WhiteHOrate, AssistedIncRate,
                                           rest_per_sqmi, number_of_bathrooms)) #have to edit later so the regressions are different
...
```{r Income and Race}
grid.arrange(ncol = 2,
  ggplot() + geom_sf(data = na.omit(acsTractsPHL.2021), aes(fill = WhiteHOrate)) +
  #  scale_fill(values = c("#25CB10", "#FA7800"), name="Race Context") +
    labs(title = "Race Context") +
    mapTheme() + theme(legend.position="bottom"), 
  ggplot() + geom_sf(data = na.omit(acsTractsPHL.2021), aes(fill = medHHInc)) +
 #   scale_fill(values = c("#25CB10", "#FA7800"), name="Income Context") +
