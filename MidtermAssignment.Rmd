...
## Residual Scatterplot (relook at)

Spatial Lag is the concept that depicts the relationship between the variables we are interested in and having that in a neighborhood context. The Spatial Lag is used to learnt the effect of how nearby house prices can change a house. The lag error is the error the values "neighbors" that surround said value. 

The scatterplot below shows a postive correlation because of negative values seen by the points outside the cluster, where it underestimates houses of extremely high values, while only a few, the outliers underestimate certain houses by $1 million dolalrs or more. What we can conclude from this graph is the model is going to have a more difficult time predicting home prices for houses with higher sale prices. 
...
Regardless of the model's overall accuracy and its ability to predict house prices. We can highlight the model's weaknesses per neighborhood and understand which areas it will over or underestimate. One of the biggest highlights that come from the map is that the model's largest errors are in the lower income suburban sections of the city. We can conclude the model is less accurate in less desired neighborhoods. 
