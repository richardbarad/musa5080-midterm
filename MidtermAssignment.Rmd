...
house_data <- house_data %>%
  mutate(
    ...
    number_of_bedrooms = ifelse(number_of_bedrooms == 0,1,number_of_bedrooms))
...
```
```{r correlation_matrix, fig.height=5.5, fig.width=14}
HDFinal_nongeom %>% 
  correlate() %>% 
  autoplot() +
...
meanerrors <- Philly.test %>% 
  as.data.frame %>%
  group_by(DIST_NAME) %>%
#  st_drop_geometry() %>%
  summarize(MAE = mean(sale_price.AbsError),
